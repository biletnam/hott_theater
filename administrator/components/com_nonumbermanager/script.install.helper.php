...
class Com_NoNumberManagerInstallerScriptHelper
{
	...
	public function uninstallExtension($extname, $type = 'plugin', $folder = 'system', $show_message = true)
	{
		...
		if (JFactory::getApplication()->input->get('option') == 'com_installer' && JFactory::getApplication()->input->get('task') == 'remove')
		{
			...
			$ignore_ids = JFactory::getApplication()->input->get('cid', array(), 'array');
	...
	private function updateDownloadKey()
	{
		...
		$query = $query->clear()
			...
			->set('u.extra_query = ' . $this->db->q(''))
			->where('u.location LIKE ' . $this->db->q('http://download.nonumber.nl%'));
		$this->db->setQuery($query);
		$this->db->execute();

		$query = $query->clear()
			->update('#__update_sites as u')
			->set('u.extra_query = ' . $this->db->q('k=' . $params->key))
			->where('u.location LIKE ' . $this->db->q('http://download.nonumber.nl%'))
			->where('u.location LIKE ' . $this->db->q('%&pro=1%'));
		$this->db->setQuery($query);
		$this->db->execute();
	}
	...
	private function removeUnusedLanguageFiles()
	{
		...
		$languages = JFolder::folders(__DIR__ . '/language');
		$delete_languages = array_diff($languages, $installed_languages);

		// Remove folders
		foreach ($delete_languages as $language)
		{
			JFolder::delete($this->getMainFolder() . '/language/' . $language);
		}
	}
}
